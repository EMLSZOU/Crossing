...
- 在控制台开启 Ubuntu 的 SSH 连接：`sudo apt-get update;sudo apt-get install openssh-server`
- Mac SSH 远程连接Ubuntu   {username}代表用户名 ：`ssh {username}@10.211.55.7`或`ssh 10.211.55.7 -l {username}`
...
- 安装Python：
  ...
  - ​Ubuntu：`sudo apt-get install python`或` sudo apt-get install python3`
  - Windows：Python2注意配置PATH，Python3在安装界面勾选"自动添加到PATH"
...
- 升级pip
  ...
  ```batch
  sudo python -m pip install -U pip  # 用pip自己升级。
  ```

  如果pip升级失败，就https://pypi.python.org/pypi/pip/下载pip-*.tar.gz，解压，进入目录，
...
- 安装virtualenv
    ...
    `python2 -m pip install virtualenv`（仅Python2需要，python3有venv模块，所以不需要安装）


- 进入创建虚拟环境的文件目录，然后建立虚拟环境“djvenv”

  Python2运行  `python2 -m virtualenv djvenv`

  Python3运行  `python3 -m venv djvenv`
...
- 在虚拟环境 djvenv 安装 MySQL数据库驱动
   ...
   `python -m pip install mysql`（Python2 用 mysql模块）

   `python3 -m pip install mysqlclient`（Python3 用 mysqlclient模块）
...
5. ##### 安装配置 Apache 或 Ngnix

- 安装：`> sudo apt-get install apache2 libapache2-mod-wsgi` 

- 配置 Apache

  在文件 /etc/apache2/apache2.conf 末尾追加
...
- 切换到指定的virtual env（也就指定了Python2还是Python3）
  ...
  ├─static   # 静态文件目录，media/js/css等等
  ├─djvenv  # 本项目的Python目录，virtual env
  └─mysite # 源代码目录，相当于src
  ...
  - `python manage.py migrate`    两步让项目的修改生效
